...
async def home_page() -> None:
    ...
    with page_layout("Home", "home"):
        ...
        # ── Hero: Start Here (prominent, full-width) ──────────────────────────
        with (
            ui.card()
            .classes("w-full mb-10")
            .style(
                f"background:linear-gradient(135deg, {theme['surface']} 0%, {theme['surface_hover']} 100%);"
                f"border:2px solid {colors['primary']};border-radius:8px;"
                f"box-shadow:0 0 20px {colors['primary'] + '20'};"
            )
        ):
            with ui.row().classes("items-center gap-3 mb-5"):
                ui.icon("rocket_launch").classes("text-3xl").style(f"color: {colors['primary']};")
                ui.label("Quick Start Guide").classes("text-2xl font-bold").style(
                    f"color: {theme['text_primary']};"
                )
            ui.label("Follow these steps to get started with backtesting:").classes(
                "text-base mb-5 block"
            ).style(f"color: {theme['text_secondary']};")

            # Steps in horizontal layout (more compact)
        ...
        if candidate_runs:
            ...
            with (
                ...
                ).props("flat").classes("text-sm mt-3").style(f"color: {colors['primary']}")

        divider()

        # ── Primary Analysis (3 cards, prominent) ─────────────────────────────
        _section_label("Analysis", theme)
        with ui.grid(columns=3).classes("w-full gap-5 mb-10"):
            nav_card(
                "Run Results",
        ...
        with ui.grid(columns=2).classes("w-full gap-5 mb-10"):
            nav_card(
                "Strategy Analysis",
        ...
        with ui.row().classes("w-full gap-3 mb-10 flex-wrap"):
        ...
        if not runs:
            ...
            cols = [
                ...
                {"name": "copy", "label": "", "field": "copy", "align": "center"},
...
def _section_label(text: str, theme: dict) -> None:
    ui.label(text).classes("text-base font-semibold mb-4").style(
        f"color: {theme['text_secondary']}; text-transform: uppercase; letter-spacing: 0.1em;"
    )
...
def _compact_nav_card(title: str, desc: str, icon: str, target: str, color: str) -> None:
    """Compact nav card for operations row."""
    theme = THEME
