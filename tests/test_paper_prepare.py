...
def test_validate_walk_forward_runtime_coverage_reports_missing_dates(monkeypatch):
    seen: list[tuple[str, str, list[str]]] = []

    class _FakeDB:
        def get_symbols_with_parquet_data(self, trade_dates: list[str]):
            return {"SBIN", "TCS"}

        def get_runtime_trade_date_coverage(self, symbols: list[str], trade_date: str):
            seen.append(("coverage", trade_date, list(symbols)))
            if trade_date == "2024-03-11":
                # 1 of 2 symbols missing = 50% gap -> exceeds 5% tolerance -> blocking
                return {
        ...
        def get_table_max_trade_dates(self, tables: list[str]):
            seen.append(("tables", ",".join(tables), []))
            return {
                ...
                "intraday_day_pack": "2024-03-11",
            }

    monkeypatch.setattr(paper_prepare, "get_db", lambda: _FakeDB())

    payload = paper_prepare.validate_walk_forward_runtime_coverage(
    ...
    assert payload["effective_symbols"] == ["SBIN", "TCS"]
    # 50% gap is blocking (exceeds 5% tolerance)
    assert payload["missing_by_date"][0]["trade_date"] == "2024-03-11"
    assert payload["missing_by_date"][0]["missing_counts"] == {
        "market_day_state": 1,
        "strategy_day_state": 1,
        "intraday_day_pack": 0,
    }
    assert payload["table_max_trade_dates"]["market_day_state"] == "2024-03-10"
    assert seen[0] == ("coverage", "2024-03-11", ["SBIN", "TCS"])
