"""Tests for the paper trading CLI parser and helpers."""

from __future__ import annotations

import sys
from datetime import UTC, datetime
from types import SimpleNamespace

import polars as pl
import pytest

from scripts.paper_trading import (
    WALK_FORWARD_STANDARD_MATRIX,
    _cmd_daily_prepare,
    _cmd_walk_forward_matrix,
    _cmd_walk_forward_replay,
    _ensure_daily_session,
    _evaluate_walk_forward,
    _parse_json,
    _resolve_paper_strategy_params,
    _run_sim_variant,
    build_parser,
    iter_trade_dates,
)


def test_paper_trading_parser_supports_start_and_status() -> None:
    parser = build_parser()

    start_args = parser.parse_args(["start", "--symbols", "SBIN,TCS"])
    assert start_args.command == "start"
    assert start_args.strategy == "CPR_LEVELS"
    assert start_args.symbols == "SBIN,TCS"
    assert start_args.max_positions == 10
    assert start_args.max_position_pct == 0.1

    status_args = parser.parse_args(["status"])
    assert status_args.command == "status"
    assert status_args.summary is False

    status_summary_args = parser.parse_args(["status", "--summary"])
    assert status_summary_args.summary is True


def test_paper_trading_parser_supports_cleanup() -> None:
    parser = build_parser()

    cleanup_args = parser.parse_args(["cleanup", "--trade-date", "2026-04-01", "--apply"])

    assert cleanup_args.command == "cleanup"
    assert cleanup_args.apply is True
    assert cleanup_args.trade_date == "2026-04-01"


def test_paper_trading_parser_supports_replay() -> None:
    parser = build_parser()

    replay_args = parser.parse_args(
        [
            "replay",
            "--session-id",
            "sess-1",
            "--symbols",
            "SBIN,TCS",
            "--start-date",
            "2024-01-01",
            "--end-date",
            "2024-01-02",
        ]
    )

    assert replay_args.command == "replay"
    assert replay_args.session_id == "sess-1"
    assert replay_args.leave_active is False


def test_paper_trading_parser_supports_daily_commands() -> None:
    parser = build_parser()

    daily_prepare_args = parser.parse_args(["daily-prepare", "--trade-date", "2024-01-01"])
    assert daily_prepare_args.command == "daily-prepare"
    assert daily_prepare_args.trade_date == "2024-01-01"
    assert daily_prepare_args.all_symbols is False

    daily_replay_args = parser.parse_args(
        [
            "daily-replay",
            "--trade-date",
            "2024-01-02",
            "--symbols",
            "SBIN,RELIANCE",
            "--strategy",
            "CPR_LEVELS",
            "--session-id",
            "paper-test",
            "--multi",
        ],
    )
    assert daily_replay_args.command == "daily-replay"
    assert daily_replay_args.session_id == "paper-test"
    assert daily_replay_args.strategy == "CPR_LEVELS"
    assert daily_replay_args.all_symbols is False
    assert daily_replay_args.multi is True

    daily_live_args = parser.parse_args(
        [
            "daily-live",
            "--trade-date",
            "2024-01-03",
            "--symbols",
            "SBIN,RELIANCE",
        ]
    )
    assert daily_live_args.command == "daily-live"
    assert daily_live_args.trade_date == "2024-01-03"
    assert daily_live_args.all_symbols is False
    assert daily_live_args.skip_coverage is False
    assert daily_live_args.direction is None
    assert daily_live_args.min_price is None
    assert daily_live_args.cpr_min_close_atr is None
    assert daily_live_args.cpr_scale_out_pct is None
    assert daily_live_args.narrowing_filter is False
    assert daily_live_args.standard_sizing is False
    assert daily_live_args.risk_based_sizing is True
    assert daily_live_args.no_alerts is False

    daily_live_no_alerts_args = parser.parse_args(
        [
            "daily-live",
            "--trade-date",
            "2024-01-03",
            "--symbols",
            "SBIN",
            "--no-alerts",
        ]
    )
    assert daily_live_no_alerts_args.no_alerts is True

    preset_live_args = parser.parse_args(
        [
            "daily-live",
            "--trade-date",
            "2024-01-03",
            "--symbols",
            "SBIN",
            "--preset",
            "CPR_LEVELS_RISK_LONG",
        ]
    )
    assert preset_live_args.preset == "CPR_LEVELS_RISK_LONG"

    daily_live_skip_args = parser.parse_args(
        ["daily-live", "--trade-date", "2024-01-03", "--all-symbols", "--skip-coverage"]
    )
    assert daily_live_skip_args.skip_coverage is True

    tuned_live_args = parser.parse_args(
        [
            "daily-live",
            "--trade-date",
            "2024-01-03",
            "--symbols",
            "SBIN",
            "--direction",
            "SHORT",
            "--skip-rvol",
            "--min-price",
            "50",
            "--cpr-min-close-atr",
            "0.35",
            "--cpr-scale-out-pct",
            "0.8",
            "--narrowing-filter",
        ]
    )
    assert tuned_live_args.direction == "SHORT"
    assert tuned_live_args.skip_rvol is True
    assert tuned_live_args.min_price == 50.0
    assert tuned_live_args.cpr_min_close_atr == 0.35
    assert tuned_live_args.cpr_scale_out_pct == 0.8
    assert tuned_live_args.narrowing_filter is True
    assert tuned_live_args.standard_sizing is False
    assert tuned_live_args.risk_based_sizing is True

    standard_sizing_args = parser.parse_args(
        [
            "daily-live",
            "--trade-date",
            "2024-01-03",
            "--symbols",
            "SBIN",
            "--standard-sizing",
        ]
    )
    assert standard_sizing_args.standard_sizing is True
    assert standard_sizing_args.risk_based_sizing is False

    timed_live_args = parser.parse_args(
        [
            "daily-live",
            "--trade-date",
            "2024-01-03",
            "--symbols",
            "SBIN",
            "--or-minutes",
            "10",
            "--entry-window-end",
            "15:00",
            "--time-exit",
            "15:00",
            "--cpr-entry-start",
            "14:00",
        ]
    )
    assert timed_live_args.or_minutes == 10
    assert timed_live_args.entry_window_end == "15:00"
    assert timed_live_args.time_exit == "15:00"
    assert timed_live_args.cpr_entry_start == "14:00"


class _DummyLock:
    def __enter__(self):
        return None

    def __exit__(self, exc_type, exc, tb):
        return False


def test_paper_trading_parser_supports_daily_replay_no_alerts() -> None:
    parser = build_parser()

    replay_args = parser.parse_args(["daily-replay", "--trade-date", "2024-01-02", "--no-alerts"])

    assert replay_args.command == "daily-replay"
    assert replay_args.no_alerts is True


def test_walk_forward_standard_matrix_uses_cpr_canonical_params() -> None:
    cpr_variants = {
        label: params
        for label, strategy, params in WALK_FORWARD_STANDARD_MATRIX
        if strategy == "CPR_LEVELS"
    }

    assert cpr_variants["CPR_LEVELS_LONG"] == {
        "direction_filter": "LONG",
        "min_price": 50,
        "cpr_min_close_atr": 0.5,
        "narrowing_filter": True,
        "risk_based_sizing": True,
        "skip_rvol_check": False,
    }
    assert cpr_variants["CPR_LEVELS_SHORT"] == {
        "direction_filter": "SHORT",
        "skip_rvol_check": True,
        "min_price": 50,
        "cpr_min_close_atr": 0.5,
        "narrowing_filter": True,
        "risk_based_sizing": True,
    }


def test_parse_json_accepts_powershell_style_object() -> None:
    parsed = _parse_json(
        "{direction_filter:LONG,skip_rvol_check:true,cpr_min_close_atr:0.35,min_price:50}"
    )

    assert parsed == {
        "direction_filter": "LONG",
        "skip_rvol_check": True,
        "cpr_min_close_atr": 0.35,
        "min_price": 50,
    }


def test_resolve_paper_strategy_params_preserves_backtest_defaults() -> None:
    cpr_long = _resolve_paper_strategy_params("CPR_LEVELS", None)
    assert cpr_long["direction_filter"] == "BOTH"
    assert "min_price" not in cpr_long
    assert "cpr_min_close_atr" not in cpr_long
    assert "scale_out_pct" not in cpr_long.get("cpr_levels_config", {})
    assert "narrowing_filter" not in cpr_long
    assert "skip_rvol_check" not in cpr_long
    assert "risk_based_sizing" not in cpr_long

    cpr_short = _resolve_paper_strategy_params("CPR_LEVELS", '{"direction_filter":"SHORT"}')
    assert cpr_short["direction_filter"] == "SHORT"
    assert "narrowing_filter" not in cpr_short
    assert "skip_rvol_check" not in cpr_short

    with pytest.raises(ValueError, match="not supported for paper workflows"):
        _resolve_paper_strategy_params("FBR", '{"direction_filter":"SHORT"}')

    preset = SimpleNamespace(preset="CPR_LEVELS_RISK_LONG")
    preset_params = _resolve_paper_strategy_params("CPR_LEVELS", None, preset)
    assert preset_params["direction_filter"] == "LONG"
    assert preset_params["min_price"] == 50
    assert preset_params["cpr_min_close_atr"] == 0.5
    assert preset_params["narrowing_filter"] is True
    assert preset_params["risk_based_sizing"] is True


def test_resolve_paper_strategy_params_standard_preset_not_clobbered_by_default() -> None:
    """Regression: paper CLI default risk_based_sizing=True must not override STANDARD preset False.

    In the daily-live parser, risk_based_sizing has default=True.  When --preset
    CPR_LEVELS_STANDARD_LONG is used, _collect_strategy_cli_overrides must NOT inject
    risk_based_sizing=True (the default) over the preset's risk_based_sizing=False.
    """
    parser = build_parser()
    args = parser.parse_args(
        [
            "daily-live",
            "--trade-date",
            "2024-01-03",
            "--symbols",
            "SBIN",
            "--preset",
            "CPR_LEVELS_STANDARD_LONG",
        ]
    )
    # Confirm that the parser default is True (no explicit --risk-based-sizing was passed)
    assert args.risk_based_sizing is True
    assert args.standard_sizing is False

    params = _resolve_paper_strategy_params("CPR_LEVELS", None, args)
    assert params["risk_based_sizing"] is False, (
        "STANDARD preset risk_based_sizing=False was overridden by CLI default True"
    )


def test_resolve_paper_strategy_params_rejects_fbr_preset() -> None:
    args = SimpleNamespace(
        preset="FBR_RISK_LONG",
        direction="BOTH",
        skip_rvol=False,
        standard_sizing=False,
        risk_based_sizing=True,
        min_price=None,
        cpr_min_close_atr=None,
        cpr_scale_out_pct=None,
        failure_window=None,
        or_minutes=None,
        entry_window_end=None,
        time_exit=None,
        cpr_entry_start=None,
        fbr_entry_window_end=None,
    )

    with pytest.raises(ValueError, match="not supported for paper workflows"):
        _resolve_paper_strategy_params("FBR", None, args)


def test_resolve_paper_strategy_params_accepts_standard_sizing_cli_override() -> None:
    args = SimpleNamespace(
        direction=None,
        skip_rvol=False,
        standard_sizing=True,
        risk_based_sizing=False,
        min_price=None,
        cpr_min_close_atr=None,
        cpr_scale_out_pct=None,
        failure_window=None,
        or_minutes=None,
        entry_window_end=None,
        time_exit=None,
        cpr_entry_start=None,
        fbr_entry_window_end=None,
    )

    cpr = _resolve_paper_strategy_params("CPR_LEVELS", None, args)
    assert cpr["risk_based_sizing"] is False


def test_resolve_paper_strategy_params_accepts_timing_cli_overrides() -> None:
    args = SimpleNamespace(
        direction="LONG",
        skip_rvol=False,
        min_price=None,
        cpr_min_close_atr=None,
        cpr_scale_out_pct=None,
        failure_window=None,
        or_minutes=15,
        entry_window_end="15:00",
        time_exit="15:05",
        cpr_entry_start="14:00",
    )

    cpr = _resolve_paper_strategy_params("CPR_LEVELS", None, args)
    assert cpr["direction_filter"] == "LONG"
    assert cpr["or_minutes"] == 15
    assert cpr["entry_window_end"] == "15:00"
    assert cpr["time_exit"] == "15:05"
    assert cpr["cpr_levels_config"]["cpr_entry_start"] == "14:00"

    with pytest.raises(ValueError, match="not supported for paper workflows"):
        _resolve_paper_strategy_params("FBR", None, args)


@pytest.mark.asyncio
async def test_cmd_daily_prepare_runs_readiness_gate(monkeypatch: pytest.MonkeyPatch) -> None:
    import scripts.paper_trading as pt

    calls: list[object] = []

    class _FakeCon:
        def execute(self, query: str, params: list[object]):
            # Simulate intraday data existing for the requested date → mode="replay"
            return SimpleNamespace(fetchone=lambda: (1,))

    class _FakeDB:
        con = _FakeCon()

        def con_execute(self, *a, **kw):
            return SimpleNamespace(fetchone=lambda: (1,))

    def fake_prepare_runtime_for_daily_paper(*, trade_date: str, symbols: list[str], mode: str):
        calls.append(("prepare", trade_date, list(symbols), mode))
        return {
            "trade_date": trade_date,
            "requested_symbols": symbols,
            "coverage_ready": True,
            "coverage": {"market_day_state": [], "strategy_day_state": [], "intraday_day_pack": []},
        }

    def fake_build_trade_date_readiness_report(trade_date: str):
        calls.append(("dq", trade_date))
        return {
            "trade_date": trade_date,
            "requested_symbols": ["SBIN"],
            "freshness_rows": [],
            "freshness_comparisons": [],
            "setup_capable_symbols": ["SBIN"],
            "late_starting_symbols": [],
            "setup_query_failed": False,
            "coverage": {"market_day_state": [], "strategy_day_state": [], "intraday_day_pack": []},
            "ready": False,
        }

    def fake_print_trade_date_readiness_report(report: dict[str, object]) -> None:
        calls.append(("print", report["trade_date"]))

    monkeypatch.setattr(pt, "get_db", lambda: _FakeDB())
    monkeypatch.setattr(pt, "get_dashboard_db", lambda: _FakeDB())
    monkeypatch.setattr(pt, "prepare_runtime_for_daily_paper", fake_prepare_runtime_for_daily_paper)
    monkeypatch.setattr(
        pt._data_quality,
        "build_trade_date_readiness_report",
        fake_build_trade_date_readiness_report,
    )
    monkeypatch.setattr(
        pt._data_quality,
        "print_trade_date_readiness_report",
        fake_print_trade_date_readiness_report,
    )
    monkeypatch.setattr(pt, "resolve_trade_date", lambda value: value)
    monkeypatch.setattr(
        pt,
        "resolve_prepare_symbols",
        lambda symbols, raw, all_symbols=False, read_only=True: symbols,
    )

    with pytest.raises(SystemExit) as excinfo:
        await _cmd_daily_prepare(
            SimpleNamespace(
                trade_date="2024-01-01",
                symbols="SBIN",
                all_symbols=False,
            )
        )

    assert excinfo.value.code == 1
    assert calls == [
        ("prepare", "2024-01-01", ["SBIN"], "replay"),
        ("dq", "2024-01-01"),
        ("print", "2024-01-01"),
    ]


@pytest.mark.asyncio
async def test_cmd_daily_replay_restores_alert_suppression_on_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    calls: list[bool] = []

    class _LockCtx:
        def __enter__(self):
            return None

        def __exit__(self, exc_type, exc, tb):
            return False

    monkeypatch.setattr(pt, "acquire_command_lock", lambda *args, **kwargs: _LockCtx())
    monkeypatch.setattr(pt, "_resolve_cli_symbols", lambda *args, **kwargs: ["SBIN"])
    monkeypatch.setattr(
        pt,
        "_resolve_paper_strategy_params",
        lambda *args, **kwargs: {"direction_filter": "LONG", "skip_rvol_check": False},
    )
    monkeypatch.setattr(pt, "pre_filter_symbols_for_strategy", lambda *args, **kwargs: ["SBIN"])
    monkeypatch.setattr(pt, "set_alerts_suppressed", lambda value: calls.append(value))

    async def fake_run_daily_workflow(**kwargs):
        raise RuntimeError("boom")

    monkeypatch.setattr(pt, "_run_daily_workflow", fake_run_daily_workflow)

    with pytest.raises(RuntimeError):
        await pt._cmd_daily_replay(
            SimpleNamespace(
                trade_date="2024-01-02",
                symbols="SBIN",
                all_symbols=False,
                strategy="CPR_LEVELS",
                strategy_params=None,
                session_id="paper-test",
                notes=None,
                leave_active=False,
                no_alerts=True,
            )
        )

    assert calls == [True, False]


@pytest.mark.asyncio
@pytest.mark.parametrize("no_alerts, expected_calls", [(False, []), (True, [True, False])])
async def test_cmd_daily_live_local_feed_alert_toggle(
    monkeypatch: pytest.MonkeyPatch,
    no_alerts: bool,
    expected_calls: list[bool],
) -> None:
    import engine.local_ticker_adapter as lta
    import scripts.paper_trading as pt

    calls: list[bool] = []
    workflow_calls: dict[str, object] = {}

    class _FakeLocalTickerAdapter:
        def __init__(
            self,
            *,
            trade_date: str,
            symbols: list[str],
            candle_interval_minutes: int = 5,
        ) -> None:
            self.trade_date = trade_date
            self.symbols = list(symbols)
            self.candle_interval_minutes = candle_interval_minutes
            workflow_calls["local_ticker"] = {
                "trade_date": trade_date,
                "symbols": list(symbols),
                "candle_interval_minutes": candle_interval_minutes,
            }

    async def fake_run_daily_workflow(**kwargs):
        workflow_calls["workflow"] = kwargs
        return {"status": "LIVE"}

    monkeypatch.setattr(pt, "resolve_trade_date", lambda value: value)
    monkeypatch.setattr(pt, "_resolve_cli_symbols", lambda *args, **kwargs: ["SBIN"])
    monkeypatch.setattr(
        pt,
        "_resolve_paper_strategy_params",
        lambda *args, **kwargs: {"direction_filter": "LONG", "skip_rvol_check": False},
    )
    monkeypatch.setattr(pt, "pre_filter_symbols_for_strategy", lambda *args, **kwargs: ["SBIN"])
    monkeypatch.setattr(pt, "set_alerts_suppressed", lambda value: calls.append(value))
    monkeypatch.setattr(lta, "LocalTickerAdapter", _FakeLocalTickerAdapter)
    monkeypatch.setattr(pt, "_run_daily_workflow", fake_run_daily_workflow)

    await pt._cmd_daily_live(
        SimpleNamespace(
            trade_date="2026-04-09",
            symbols="SBIN",
            all_symbols=False,
            strategy="CPR_LEVELS",
            strategy_params=None,
            session_id="paper-test",
            notes=None,
            skip_coverage=True,
            poll_interval_sec=1.0,
            candle_interval_minutes=5,
            max_cycles=1,
            complete_on_exit=False,
            feed_source="local",
            no_alerts=no_alerts,
            multi=False,
        )
    )

    assert calls == expected_calls
    assert workflow_calls["local_ticker"] == {
        "trade_date": "2026-04-09",
        "symbols": ["SBIN"],
        "candle_interval_minutes": 5,
    }
    assert workflow_calls["workflow"]["strategy_params"]["feed_source"] == "local"
    assert workflow_calls["workflow"]["live_kwargs"]["ticker_adapter"].trade_date == "2026-04-09"


@pytest.mark.asyncio
async def test_run_sim_variant_stamps_paper_execution_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    import scripts.paper_trading as pt
    from engine.cpr_atr_strategy import BacktestParams

    calls: list[tuple[str, object]] = []

    class _FakeBacktest:
        def __init__(self, params, db):
            self.params = params
            self.db = db

        def run(self, *, symbols, start, end, verbose, use_cache=True):
            calls.append(("run", list(symbols), start, end, verbose, use_cache))
            return SimpleNamespace(
                run_id="bt-paper-base",
                params=BacktestParams(strategy="CPR_LEVELS"),
                df=pl.DataFrame(
                    {
                        "run_id": ["bt-paper-base"],
                        "symbol": ["SBIN"],
                        "trade_date": ["2024-01-02"],
                        "profit_loss": [123.45],
                        "exit_reason": ["TARGET"],
                    }
                ),
                run_context={
                    "start_date": "2024-01-02",
                    "end_date": "2024-01-02",
                    "symbols": ["SBIN"],
                },
            )

    class _FakeDB:
        class _FakeCon:
            def execute(self, *args, **kwargs):
                return None

            def register(self, *args, **kwargs):
                return None

            def unregister(self, *args, **kwargs):
                return None

        con = _FakeCon()

        def store_run_metadata(self, **kwargs):
            calls.append(("metadata", kwargs["execution_mode"]))

        def store_backtest_results(
            self,
            results_df,
            execution_mode: str | None = None,
            transactional: bool = True,
        ):
            calls.append(("results", execution_mode, results_df["run_id"][0]))
            assert execution_mode == "PAPER"
            assert results_df["run_id"][0] != "bt-paper-base"
            return results_df.height

    monkeypatch.setattr(pt, "CPRATRBacktest", _FakeBacktest)
    monkeypatch.setattr(pt, "get_db", lambda: _FakeDB())
    monkeypatch.setitem(
        sys.modules,
        "web.state",
        SimpleNamespace(
            invalidate_run_cache=lambda run_id=None: calls.append(("invalidate", run_id))
        ),
    )

    result = _run_sim_variant(
        trade_date="2024-01-02",
        symbols=["SBIN"],
        strategy="CPR_LEVELS",
        strategy_params={"direction_filter": "LONG"},
        force=True,
    )

    assert result["run_id"] != "bt-paper-base"
    assert result["trades"] == 1
    assert calls[0][0] == "run"
    assert calls[1] == ("metadata", "PAPER")
    assert calls[2][0] == "results"
    assert calls[2][1] == "PAPER"
    assert calls[3] == ("invalidate", result["run_id"])


@pytest.mark.asyncio
async def test_cmd_daily_sim_holds_runtime_writer_lock(monkeypatch: pytest.MonkeyPatch) -> None:
    import scripts.paper_trading as pt

    calls: list[str] = []

    class _LockCtx:
        def __enter__(self):
            calls.append("enter")
            return None

        def __exit__(self, exc_type, exc, tb):
            calls.append("exit")
            return False

    def fake_run_sim_variant(**kwargs):
        calls.append(f"run:{kwargs['strategy']}")
        return {
            "run_id": "paper-123",
            "strategy": kwargs["strategy"],
            "strategy_params": kwargs["strategy_params"],
            "trade_date": kwargs["trade_date"],
            "symbol_count": len(kwargs["symbols"]),
            "trades": 1,
            "wins": 1,
            "win_rate": 100.0,
            "total_pnl": 1.0,
            "elapsed_sec": 0.1,
        }

    monkeypatch.setattr(pt, "acquire_command_lock", lambda *args, **kwargs: _LockCtx())
    monkeypatch.setattr(pt, "_resolve_cli_symbols", lambda *args, **kwargs: ["SBIN"])
    monkeypatch.setattr(
        pt,
        "_resolve_paper_strategy_params",
        lambda *args, **kwargs: {"direction_filter": "LONG", "skip_rvol_check": False},
    )
    monkeypatch.setattr(pt, "_run_sim_variant", fake_run_sim_variant)

    await pt._cmd_daily_sim(
        SimpleNamespace(
            trade_date="2024-01-02",
            symbols=None,
            all_symbols=True,
            strategy="CPR_LEVELS",
            strategy_params=None,
            force=True,
            direction=None,
            skip_rvol=False,
            min_price=None,
            cpr_min_close_atr=None,
            narrowing_filter=False,
            failure_window=None,
            or_minutes=None,
            entry_window_end=None,
            time_exit=None,
            cpr_entry_start=None,
            fbr_entry_window_end=None,
        )
    )

    assert calls == ["enter", "run:CPR_LEVELS", "exit"]


def test_paper_trading_parser_supports_walk_forward() -> None:
    parser = build_parser()
    walk_forward_args = parser.parse_args(
        [
            "walk-forward",
            "--start-date",
            "2024-01-02",
            "--end-date",
            "2024-01-03",
            "--strategy",
            "CPR_LEVELS",
        ]
    )
    assert walk_forward_args.command == "walk-forward"
    assert walk_forward_args.start_date == "2024-01-02"
    assert walk_forward_args.end_date == "2024-01-03"
    assert walk_forward_args.force is False
    assert walk_forward_args.all_symbols is False


def test_paper_trading_parser_supports_walk_forward_matrix() -> None:
    parser = build_parser()
    matrix_args = parser.parse_args(
        [
            "walk-forward-matrix",
            "--start-date",
            "2024-01-02",
            "--end-date",
            "2024-01-03",
            "--all-symbols",
        ]
    )
    assert matrix_args.command == "walk-forward-matrix"
    assert matrix_args.start_date == "2024-01-02"
    assert matrix_args.end_date == "2024-01-03"
    assert matrix_args.all_symbols is True
    assert matrix_args.force is False


def test_paper_trading_parser_supports_walk_forward_replay() -> None:
    parser = build_parser()
    replay_args = parser.parse_args(
        [
            "walk-forward-replay",
            "--start-date",
            "2024-01-02",
            "--end-date",
            "2024-01-03",
            "--strategy",
            "CPR_LEVELS",
        ]
    )
    assert replay_args.command == "walk-forward-replay"
    assert replay_args.strategy == "CPR_LEVELS"
    assert replay_args.force is False


def test_paper_trading_parser_walk_forward_force_flag() -> None:
    parser = build_parser()
    args = parser.parse_args(
        ["walk-forward", "--start-date", "2024-01-02", "--end-date", "2024-01-03", "--force"]
    )
    assert args.force is True


def test_paper_trading_parser_supports_all_symbols_flag() -> None:
    parser = build_parser()
    args = parser.parse_args(
        ["walk-forward", "--start-date", "2024-01-02", "--end-date", "2024-01-03", "--all-symbols"]
    )
    assert args.all_symbols is True


def test_resolve_cli_symbols_uses_parsed_list_only(monkeypatch: pytest.MonkeyPatch) -> None:
    import scripts.paper_trading as pt

    seen: dict[str, object] = {}

    def fake_resolve_prepare_symbols(
        symbols, symbols_csv, *, all_symbols: bool = False, read_only: bool = True
    ) -> list[str]:
        seen["symbols"] = symbols
        seen["symbols_csv"] = symbols_csv
        seen["all_symbols"] = all_symbols
        seen["read_only"] = read_only
        return ["SBIN", "RELIANCE"]

    monkeypatch.setattr(pt, "resolve_prepare_symbols", fake_resolve_prepare_symbols)

    parser = build_parser()
    args = parser.parse_args(["daily-live", "--trade-date", "2024-01-02", "--symbols", "SBIN"])

    assert pt._resolve_cli_symbols(parser, args) == ["SBIN", "RELIANCE"]
    assert seen == {
        "symbols": ["SBIN"],
        "symbols_csv": None,
        "all_symbols": False,
        "read_only": True,
    }


@pytest.mark.asyncio
async def test_cmd_walk_forward_matrix_invokes_standard_cpr_runs(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    calls: list[dict[str, object]] = []

    async def fake_run_walk_forward_validation(**kwargs):
        calls.append(kwargs)
        return {
            "wf_run_id": f"wf-{len(calls)}",
            "decision": {"status": "PASS", "reasons": []},
            "summary": {"replayed_days": 2, "days_requested": 2},
            "gate_key": f"gate-{len(calls)}",
            "scope_key": "ALL:2",
            "trade_dates": ["2024-01-02", "2024-01-03"],
        }

    monkeypatch.setattr(pt, "run_walk_forward_validation", fake_run_walk_forward_validation)

    async def fake_validate_walk_forward_runtime_preflight(**kwargs):
        return {"ready": True, "coverage_ready": True, "schema_ready": True}

    monkeypatch.setattr(
        pt, "_validate_walk_forward_runtime_preflight", fake_validate_walk_forward_runtime_preflight
    )
    monkeypatch.setattr(pt, "resolve_trade_date", lambda value: value)
    monkeypatch.setattr(pt, "_parse_symbols_arg", lambda value: None)
    monkeypatch.setattr(
        pt,
        "resolve_prepare_symbols",
        lambda symbols, raw, all_symbols=False, read_only=True: (
            ["SBIN", "RELIANCE"] if all_symbols else symbols
        ),
    )
    monkeypatch.setattr(pt, "acquire_command_lock", lambda *args, **kwargs: _DummyLock())

    await _cmd_walk_forward_matrix(
        SimpleNamespace(
            start_date="2024-01-02",
            end_date="2024-01-03",
            symbols=None,
            all_symbols=True,
            notes=None,
            force=False,
            carry_forward_equity=False,
            gate_min_days=None,
            gate_min_trades=None,
            gate_min_profitable_ratio=None,
            gate_pf_floor=None,
            gate_max_daily_loss=None,
        )
    )

    assert len(calls) == 2
    assert [call["strategy"] for call in calls] == ["CPR_LEVELS", "CPR_LEVELS"]
    assert [call["strategy_params"]["direction_filter"] for call in calls] == [
        "LONG",
        "SHORT",
    ]
    assert calls[0]["strategy_params"]["min_price"] == 50
    assert calls[0]["strategy_params"]["cpr_min_close_atr"] == 0.5
    assert calls[0]["strategy_params"]["narrowing_filter"] is True
    assert calls[0]["strategy_params"]["skip_rvol_check"] is False
    assert calls[1]["strategy_params"]["min_price"] == 50
    assert calls[1]["strategy_params"]["cpr_min_close_atr"] == 0.5
    assert calls[1]["strategy_params"]["narrowing_filter"] is True
    assert calls[1]["strategy_params"]["skip_rvol_check"] is True
    assert all(call["all_symbols"] is True for call in calls)
    assert all(call["symbols"] == ["SBIN", "RELIANCE"] for call in calls)


@pytest.mark.asyncio
async def test_cmd_walk_forward_cleanup_builds_and_applies_plan(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    plans: list[dict[str, object]] = []
    market_deletes: list[list[str]] = []
    backtest_deletes: list[list[str]] = []
    pg_deletes: list[str] = []

    async def fake_build_walk_forward_cleanup_plan(**kwargs):
        plans.append(kwargs)
        return {
            "wf_runs": [{"wf_run_id": "wf-1", "exists": True, "fold_count": 0}],
            "explicit_run_ids": ["run-legacy"],
            "tagged_run_ids": ["run-tagged"],
            "run_ids_to_delete": ["run-legacy", "run-tagged"],
            "duckdb_counts": {
                "backtest_results": 2,
                "run_daily_pnl": 2,
                "run_metrics": 2,
                "run_metadata": 2,
            },
        }

    class _FakeDb:
        def delete_runs(self, run_ids):
            market_deletes.append(list(run_ids))
            return {"backtest_results": 2, "run_daily_pnl": 2, "run_metrics": 2, "run_metadata": 2}

    class _FakeBacktestDb:
        def delete_runs(self, run_ids):
            backtest_deletes.append(list(run_ids))
            return {"backtest_results": 2, "run_daily_pnl": 2, "run_metrics": 2, "run_metadata": 2}

    async def fake_delete_walk_forward_run(wf_run_id: str):
        pg_deletes.append(wf_run_id)
        return 1

    monkeypatch.setattr(
        pt, "_build_walk_forward_cleanup_plan", fake_build_walk_forward_cleanup_plan
    )
    monkeypatch.setattr(pt, "delete_walk_forward_run", fake_delete_walk_forward_run)
    monkeypatch.setitem(sys.modules, "db.duckdb", SimpleNamespace(get_db=lambda: _FakeDb()))
    monkeypatch.setitem(
        sys.modules,
        "db.backtest_db",
        SimpleNamespace(get_backtest_db=lambda: _FakeBacktestDb()),
    )
    monkeypatch.setattr(pt, "acquire_command_lock", lambda *args, **kwargs: _DummyLock())

    await pt._cmd_walk_forward_cleanup(
        SimpleNamespace(
            wf_run_id=["wf-1"],
            run_id=["run-legacy"],
            apply=True,
        )
    )

    assert plans[0]["wf_run_ids"] == ["wf-1"]
    assert plans[0]["explicit_run_ids"] == ["run-legacy"]
    assert market_deletes == [["run-legacy", "run-tagged"]]
    assert backtest_deletes == [["run-legacy", "run-tagged"]]
    assert pg_deletes == ["wf-1"]


def test_paper_trading_parser_supports_walk_forward_cleanup() -> None:
    parser = build_parser()

    args = parser.parse_args(
        [
            "walk-forward-cleanup",
            "--wf-run-id",
            "wf-1",
            "--run-id",
            "run-1",
            "--apply",
        ]
    )

    assert args.command == "walk-forward-cleanup"
    assert args.wf_run_id == ["wf-1"]
    assert args.run_id == ["run-1"]
    assert args.apply is True


@pytest.mark.asyncio
async def test_cmd_walk_forward_replay_invokes_replay_runner(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    calls: list[dict[str, object]] = []

    async def fake_run_walk_forward_replay(**kwargs):
        calls.append(kwargs)
        return {"wf_run_id": "wf-replay"}

    monkeypatch.setattr(pt, "run_walk_forward_replay", fake_run_walk_forward_replay)
    monkeypatch.setattr(pt, "resolve_trade_date", lambda value: value)
    monkeypatch.setattr(pt, "_parse_symbols_arg", lambda value: ["SBIN"])
    monkeypatch.setattr(
        pt,
        "resolve_prepare_symbols",
        lambda symbols, raw, all_symbols=False, read_only=True: symbols,
    )
    monkeypatch.setattr(pt, "acquire_command_lock", lambda *args, **kwargs: _DummyLock())

    await _cmd_walk_forward_replay(
        SimpleNamespace(
            start_date="2024-01-02",
            end_date="2024-01-03",
            symbols="SBIN",
            all_symbols=False,
            strategy="CPR_LEVELS",
            strategy_params=None,
            notes=None,
            force=False,
        )
    )

    assert calls[0]["strategy"] == "CPR_LEVELS"
    assert calls[0]["symbols"] == ["SBIN"]


@pytest.mark.asyncio
async def test_walk_forward_runtime_preflight_rejects_narrow_decision_schema(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    async def fake_get_schema():
        return {
            "columns": {
                "decision": {
                    "data_type": "character varying",
                    "character_maximum_length": 10,
                }
            },
            "constraints": {
                "walk_forward_runs_decision_check": "(decision = ANY (ARRAY['PASS'::text, 'FAIL'::text]))",
            },
        }

    monkeypatch.setattr(
        pt, "validate_walk_forward_runtime_coverage", lambda **kwargs: {"coverage_ready": True}
    )
    monkeypatch.setattr(pt, "get_walk_forward_run_schema", fake_get_schema)

    with pytest.raises(SystemExit, match="INCONCLUSIVE"):
        await pt._validate_walk_forward_runtime_preflight(
            start_date="2026-03-23",
            end_date="2026-03-25",
            symbols=["SBIN"],
        )


@pytest.mark.asyncio
async def test_walk_forward_replay_skips_checkpointed_days(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt
    from db.postgres import WalkForwardFold

    checkpoint_fold = WalkForwardFold(
        fold_id=1,
        wf_run_id="wf-replay-cpr_levels-2026-03-23-2026-03-25-abc123",
        fold_index=1,
        trade_date="2026-03-23",
        status="COMPLETED",
        reference_run_id="paper-cpr_levels-2026-03-23",
        paper_session_id="paper-cpr_levels-2026-03-23",
        total_trades=1,
        total_pnl=10.0,
        total_return_pct=1.0,
        summary_json={"checkpoint": True},
        parity_actual_run_id=None,
        parity_status=None,
        created_at=datetime(2026, 3, 27, tzinfo=UTC),
        updated_at=datetime(2026, 3, 27, tzinfo=UTC),
    )

    folds: list[WalkForwardFold] = [checkpoint_fold]
    calls: list[str] = []
    fold_writes: list[dict[str, object]] = []
    run_updates: list[dict[str, object]] = []

    async def fake_validate_walk_forward_runtime_preflight(**kwargs):
        return {"ready": True, "coverage_ready": True, "schema_ready": True, "coverage": {}}

    async def fake_create_walk_forward_run(**kwargs):
        return None

    async def fake_get_session(session_id: str):
        return None

    async def fake_list_walk_forward_folds(wf_run_id: str):
        return list(folds)

    async def fake_upsert_walk_forward_fold(**kwargs):
        fold_writes.append(kwargs)
        folds.append(
            WalkForwardFold(
                fold_id=len(folds) + 1,
                wf_run_id=kwargs["wf_run_id"],
                fold_index=kwargs["fold_index"],
                trade_date=kwargs["trade_date"],
                status=kwargs["status"],
                reference_run_id=kwargs["reference_run_id"],
                paper_session_id=kwargs["paper_session_id"],
                total_trades=int(kwargs["total_trades"]),
                total_pnl=float(kwargs["total_pnl"]),
                total_return_pct=float(kwargs["total_return_pct"]),
                summary_json=dict(kwargs["summary_json"]),
                parity_actual_run_id=None,
                parity_status=None,
                created_at=checkpoint_fold.created_at,
                updated_at=checkpoint_fold.updated_at,
            )
        )
        return folds[-1]

    async def fake_run_daily_workflow(**kwargs):
        calls.append(kwargs["trade_date"])
        return {
            "session_id": f"paper-cpr_levels-{kwargs['trade_date']}",
            "preparation": {"coverage_ready": True},
            "replay": {"archive": {"rows": 2}},
        }

    def fake_summarize_walk_forward_sessions(session_ids, portfolio_value):
        return {
            "replayed_days": len(session_ids),
            "sessions_with_trades": len(session_ids),
            "total_pnl": 123.45,
            "total_return_pct": 1.2345,
            "avg_daily_return_pct": 1.2345,
            "profitable_days": 1,
            "profitable_days_ratio": 1.0,
            "worst_daily_return_pct": 1.2345,
            "best_daily_return_pct": 1.2345,
            "profit_factor": 1.0,
        }

    async def fake_update_walk_forward_run(wf_run_id: str, **kwargs):
        run_updates.append({"wf_run_id": wf_run_id, **kwargs})
        return None

    monkeypatch.setattr(
        pt, "_validate_walk_forward_runtime_preflight", fake_validate_walk_forward_runtime_preflight
    )
    monkeypatch.setattr(pt, "create_walk_forward_run", fake_create_walk_forward_run)
    monkeypatch.setattr(pt, "get_session", fake_get_session)
    monkeypatch.setattr(pt, "list_walk_forward_folds", fake_list_walk_forward_folds)
    monkeypatch.setattr(pt, "upsert_walk_forward_fold", fake_upsert_walk_forward_fold)
    monkeypatch.setattr(pt, "_run_daily_workflow", fake_run_daily_workflow)
    monkeypatch.setattr(
        pt, "_summarize_walk_forward_sessions", fake_summarize_walk_forward_sessions
    )
    monkeypatch.setattr(pt, "update_walk_forward_run", fake_update_walk_forward_run)
    monkeypatch.setattr(pt, "_get_archived_paper_result_run_ids", lambda session_ids: set())
    monkeypatch.setattr(
        pt, "iter_trade_dates", lambda start_date, end_date: ["2026-03-23", "2026-03-25"]
    )
    monkeypatch.setattr(pt, "make_gate_key", lambda strategy, params: "gate-1234")
    monkeypatch.setattr(pt, "make_scope_key", lambda symbols, all_symbols=False: "scope-1234")
    monkeypatch.setattr(
        pt, "_evaluate_walk_forward", lambda summary, gate=None: {"status": "PASS", "reasons": []}
    )

    result = await pt.run_walk_forward_replay(
        start_date="2026-03-23",
        end_date="2026-03-25",
        symbols=["SBIN"],
        strategy="CPR_LEVELS",
        strategy_params={},
        force=False,
    )

    assert calls == ["2026-03-25"]
    assert any(entry["status"] == "ALREADY_DONE" for entry in result["results"])
    assert any(entry["status"] == "REPLAYED" for entry in result["results"])
    assert fold_writes[0]["trade_date"] == "2026-03-25"
    assert run_updates[-1]["decision"] == "PASS"


def test_paper_trading_parser_supports_live() -> None:
    parser = build_parser()

    live_args = parser.parse_args(
        ["live", "--session-id", "sess-2", "--max-cycles", "2", "--no-alerts"]
    )

    assert live_args.command == "live"
    assert live_args.session_id == "sess-2"
    assert live_args.max_cycles == 2
    assert live_args.no_alerts is True


def test_paper_trading_main_configures_windows_asyncio(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    events: list[str] = []

    async def fake_handler(_args) -> None:
        return None

    class _FakeParser:
        def parse_args(self):
            return SimpleNamespace(handler=fake_handler)

    def fake_run(coro) -> None:
        events.append("run")
        coro.close()

    monkeypatch.setattr(pt, "configure_windows_stdio", lambda **_: events.append("stdio"))
    monkeypatch.setattr(pt, "configure_windows_asyncio", lambda: events.append("asyncio"))
    monkeypatch.setattr(pt, "run_asyncio", fake_run)
    monkeypatch.setattr(pt, "build_parser", lambda: _FakeParser())
    monkeypatch.setattr(
        pt,
        "_pdb",
        lambda: SimpleNamespace(cleanup_stale_sessions=lambda: 0),
    )

    pt.main()

    assert events == ["stdio", "asyncio", "run"]


# ── iter_trade_dates tests ──────────────────────────────────────────────


def test_iter_trade_dates_skips_weekends(monkeypatch: pytest.MonkeyPatch) -> None:
    # 2024-01-06 is Saturday, 2024-01-07 is Sunday
    import engine.walk_forward_validator as wfv

    class _FakeCon:
        def execute(self, query: str, params: list[str]):
            return SimpleNamespace(
                fetchall=lambda: [
                    ("2024-01-05",),
                    ("2024-01-08",),
                ]
            )

    class _FakeDB:
        con = _FakeCon()

    monkeypatch.setattr(wfv, "get_db", lambda: _FakeDB())
    monkeypatch.setattr(wfv, "get_dashboard_db", lambda: _FakeDB())
    dates = iter_trade_dates("2024-01-05", "2024-01-08")
    assert dates == ["2024-01-05", "2024-01-08"]


def test_iter_trade_dates_single_day(monkeypatch: pytest.MonkeyPatch) -> None:
    import engine.walk_forward_validator as wfv

    class _FakeCon:
        def execute(self, query: str, params: list[str]):
            return SimpleNamespace(fetchall=lambda: [("2024-01-03",)])

    class _FakeDB:
        con = _FakeCon()

    monkeypatch.setattr(wfv, "get_db", lambda: _FakeDB())
    monkeypatch.setattr(wfv, "get_dashboard_db", lambda: _FakeDB())
    dates = iter_trade_dates("2024-01-03", "2024-01-03")
    assert dates == ["2024-01-03"]


def test_iter_trade_dates_full_weekend_range_returns_empty(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Saturday to Sunday
    import engine.walk_forward_validator as wfv

    class _FakeCon:
        def execute(self, query: str, params: list[str]):
            return SimpleNamespace(fetchall=lambda: [])

    class _FakeDB:
        con = _FakeCon()

    monkeypatch.setattr(wfv, "get_db", lambda: _FakeDB())
    monkeypatch.setattr(wfv, "get_dashboard_db", lambda: _FakeDB())
    dates = iter_trade_dates("2024-01-06", "2024-01-07")
    assert dates == []


def test_iter_trade_dates_raises_on_reversed_range() -> None:
    with pytest.raises(ValueError, match="start_date must be <= end_date"):
        iter_trade_dates("2024-01-10", "2024-01-05")


def test_iter_trade_dates_full_week(monkeypatch: pytest.MonkeyPatch) -> None:
    import engine.walk_forward_validator as wfv

    class _FakeCon:
        def execute(self, query: str, params: list[str]):
            return SimpleNamespace(
                fetchall=lambda: [
                    ("2024-01-01",),
                    ("2024-01-02",),
                    ("2024-01-03",),
                    ("2024-01-04",),
                    ("2024-01-05",),
                ]
            )

    class _FakeDB:
        con = _FakeCon()

    monkeypatch.setattr(wfv, "get_db", lambda: _FakeDB())
    monkeypatch.setattr(wfv, "get_dashboard_db", lambda: _FakeDB())
    dates = iter_trade_dates("2024-01-01", "2024-01-07")
    assert len(dates) == 5
    assert "2024-01-06" not in dates
    assert "2024-01-07" not in dates


def test_iter_trade_dates_prefers_actual_market_sessions(monkeypatch: pytest.MonkeyPatch) -> None:
    import engine.walk_forward_validator as wfv

    class _FakeCon:
        def execute(self, query: str, params: list[str]):
            assert params == ["2026-03-01", "2026-03-20"]
            return SimpleNamespace(
                fetchall=lambda: [
                    ("2026-03-02",),
                    ("2026-03-04",),
                    ("2026-03-05",),
                ]
            )

    class _FakeDB:
        con = _FakeCon()

    monkeypatch.setattr(wfv, "get_db", lambda: _FakeDB())
    monkeypatch.setattr(wfv, "get_dashboard_db", lambda: _FakeDB())

    dates = iter_trade_dates("2026-03-01", "2026-03-20")

    assert dates == ["2026-03-02", "2026-03-04", "2026-03-05"]


def test_iter_trade_dates_falls_back_to_daily_dates_when_5min_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import engine.walk_forward_validator as wfv

    class _FakeCon:
        def execute(self, query: str, params: list[str]):
            assert params == ["2026-03-01", "2026-03-20"]
            if "FROM v_5min" in query:
                raise RuntimeError("v_5min unavailable")
            if "FROM v_daily" in query:
                return SimpleNamespace(
                    fetchall=lambda: [
                        ("2026-03-02",),
                        ("2026-03-03",),
                        ("2026-03-05",),
                    ]
                )
            raise AssertionError(f"unexpected query: {query}")

    class _FakeDB:
        con = _FakeCon()

    monkeypatch.setattr(wfv, "get_db", lambda: _FakeDB())
    monkeypatch.setattr(wfv, "get_dashboard_db", lambda: _FakeDB())

    dates = iter_trade_dates("2026-03-01", "2026-03-20")

    assert dates == ["2026-03-02", "2026-03-03", "2026-03-05"]


# ── _ensure_daily_session tests ─────────────────────────────────────────


@pytest.mark.asyncio
async def test_ensure_daily_session_creates_new_when_none_exists(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    async def fake_get_session(session_id: str):
        return None

    created: list[dict] = []

    async def fake_create_paper_session(**kwargs):
        created.append(kwargs)
        return SimpleNamespace(session_id=kwargs["session_id"], status="ACTIVE")

    monkeypatch.setattr(pt, "get_session", fake_get_session)
    monkeypatch.setattr(pt, "create_paper_session", fake_create_paper_session)

    session = await _ensure_daily_session(
        session_id=None,
        trade_date="2024-01-02",
        strategy="CPR_LEVELS",
        symbols=["SBIN"],
        strategy_params={},
        notes=None,
    )

    assert session.session_id == "paper-cpr_levels-2024-01-02"
    assert len(created) == 1
    assert created[0]["status"] == "ACTIVE"


@pytest.mark.asyncio
async def test_ensure_daily_session_creates_fresh_session_when_active_exists(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    existing = SimpleNamespace(session_id="paper-cpr_levels-2024-01-02", status="ACTIVE")

    async def fake_get_session(session_id: str):
        return existing

    create_calls: list[dict] = []

    async def fake_create_paper_session(**kwargs):
        create_calls.append(kwargs)
        return SimpleNamespace(session_id=kwargs["session_id"], status="ACTIVE")

    monkeypatch.setattr(pt, "get_session", fake_get_session)
    monkeypatch.setattr(pt, "create_paper_session", fake_create_paper_session)

    session = await _ensure_daily_session(
        session_id=None,
        trade_date="2024-01-02",
        strategy="CPR_LEVELS",
        symbols=["SBIN"],
        strategy_params={},
        notes=None,
    )

    assert session.session_id.startswith("paper-cpr_levels-2024-01-02-")
    assert len(create_calls) == 1
    assert create_calls[0]["status"] == "ACTIVE"
    assert create_calls[0]["strategy_params"] == {}


@pytest.mark.asyncio
async def test_ensure_daily_session_creates_fresh_session_when_paused_exists(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    existing = SimpleNamespace(session_id="paper-cpr_levels-2024-01-02", status="PAUSED")

    async def fake_get_session(session_id: str):
        return existing

    create_calls: list[dict] = []

    async def fake_create_paper_session(**kwargs):
        create_calls.append(kwargs)
        return SimpleNamespace(session_id=kwargs["session_id"], status="ACTIVE")

    monkeypatch.setattr(pt, "get_session", fake_get_session)
    monkeypatch.setattr(pt, "create_paper_session", fake_create_paper_session)

    session = await _ensure_daily_session(
        session_id=None,
        trade_date="2024-01-02",
        strategy="CPR_LEVELS",
        symbols=["SBIN"],
        strategy_params={},
        notes=None,
    )

    assert session.session_id.startswith("paper-cpr_levels-2024-01-02-")
    assert len(create_calls) == 1
    assert create_calls[0]["status"] == "ACTIVE"


# ── _ensure_daily_session mode/wf_run_id threading tests ────────────────


@pytest.mark.asyncio
async def test_ensure_daily_session_passes_mode_and_wf_run_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import scripts.paper_trading as pt

    async def fake_get_session(session_id: str):
        return None

    created: list[dict] = []

    async def fake_create_paper_session(**kwargs):
        created.append(kwargs)
        return SimpleNamespace(session_id=kwargs["session_id"], status="ACTIVE")

    monkeypatch.setattr(pt, "get_session", fake_get_session)
    monkeypatch.setattr(pt, "create_paper_session", fake_create_paper_session)

    await _ensure_daily_session(
        session_id=None,
        trade_date="2024-01-02",
        strategy="CPR_LEVELS",
        symbols=["SBIN"],
        strategy_params={},
        notes=None,
        mode="walk_forward",
        wf_run_id="wf-cpr_levels-2024-01-02-2024-01-31",
    )

    assert created[0]["mode"] == "walk_forward"
    assert created[0]["wf_run_id"] == "wf-cpr_levels-2024-01-02-2024-01-31"


# ── _evaluate_walk_forward tests ─────────────────────────────────────────


def test_evaluate_walk_forward_pass() -> None:
    summary = {
        "replayed_days": 10,
        "total_trades": 25,
        "sessions_with_trades": 8,
        "avg_daily_return_pct": 0.5,
        "profitable_days_ratio": 0.7,
        "worst_daily_return_pct": -3.0,
    }
    result = _evaluate_walk_forward(summary)
    assert result["status"] == "PASS"
    assert result["reasons"] == []


def test_evaluate_walk_forward_fail_non_positive_return() -> None:
    summary = {
        "replayed_days": 10,
        "total_trades": 20,
        "avg_daily_return_pct": -0.1,
        "profitable_days_ratio": 0.6,
        "worst_daily_return_pct": -5.0,
    }
    result = _evaluate_walk_forward(summary)
    assert result["status"] == "FAIL"
    assert "non_positive_average_return" in result["reasons"]


def test_evaluate_walk_forward_fail_insufficient_profitable_days() -> None:
    summary = {
        "replayed_days": 10,
        "total_trades": 20,
        "avg_daily_return_pct": 0.2,
        "profitable_days_ratio": 0.4,  # below 50% threshold
        "worst_daily_return_pct": -3.0,
    }
    result = _evaluate_walk_forward(summary)
    assert result["status"] == "FAIL"
    assert "insufficient_profitable_days" in result["reasons"]


def test_evaluate_walk_forward_fail_excessive_daily_drawdown() -> None:
    summary = {
        "replayed_days": 10,
        "total_trades": 20,
        "avg_daily_return_pct": 0.3,
        "profitable_days_ratio": 0.6,
        "worst_daily_return_pct": -11.0,  # worse than -10%
    }
    result = _evaluate_walk_forward(summary)
    assert result["status"] == "FAIL"
    assert "excessive_daily_drawdown" in result["reasons"]


def test_evaluate_walk_forward_fail_no_replayed_days() -> None:
    summary = {"replayed_days": 0}
    result = _evaluate_walk_forward(summary)
    assert result["status"] == "INCONCLUSIVE"
    assert "no_replayed_days" in result["reasons"]


def test_evaluate_walk_forward_inconclusive_few_days() -> None:
    summary = {"replayed_days": 2, "total_trades": 3}
    result = _evaluate_walk_forward(summary)
    assert result["status"] == "INCONCLUSIVE"
    assert any("insufficient_days" in r for r in result["reasons"])


def test_evaluate_walk_forward_inconclusive_few_trades() -> None:
    summary = {"replayed_days": 10, "total_trades": 3}
    result = _evaluate_walk_forward(summary)
    assert result["status"] == "INCONCLUSIVE"
    assert any("insufficient_trades" in r for r in result["reasons"])
